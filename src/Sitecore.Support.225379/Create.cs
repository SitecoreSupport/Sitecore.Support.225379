...
namespace Sitecore.Support.Shell.Framework.Commands
{
  ...
  public class Create : Command
  {
    ...
    public override void Execute(CommandContext context)
    {
      ...
      {
        templateItem = Context.ContentDatabase.Templates[@string];
    ...
    protected void Run(ClientPipelineArgs args)
    {
      ...
      if (args.IsPostBack)
      {
        if (args.HasResult)
        {
          ...
          if (@string.Length > 0)
          {
            BranchItem branch = database.Branches[@string];
          ...
          TemplateItem templateItem = database.Templates[string2];
      ...
      else
      {
        ...
        if (@string.Length > 0)
        {
          BranchItem branchItem = database.Branches[@string];
          defaultValue = branchItem.Name;
        }
        else
        {
          TemplateItem templateItem2 = database.Templates[string2];
